  ...
  <BaseModal :is-open="true" title="Invite Organization's Member" @close="$emit('close')">
    <div class="space-y-6">
      ...
      <p class="text-sm text-gray-500 flex items-start gap-2">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-blue-500 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M16 12H8m0 0l4 4m-4-4l4-4M12 4v16" />
        </svg>
        Send a role-based invite. They will receive an email with next steps.
      </p>

      <!-- Email Field -->
      <div class="group">
        <label class="flex items-center text-xs font-semibold tracking-wide text-gray-600 mb-2 uppercase">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1 text-gray-400 group-focus-within:text-blue-500 transition-colors" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <transition name="fade-in" mode="out-in">
        <div
          v-if="message"
          :key="message"
          class="flex items-start gap-3 p-3 rounded-lg border text-sm shadow-sm"
          :class="isError ? 'bg-red-50 border-red-200 text-red-700' : 'bg-green-50 border-green-200 text-green-700'"
        >
          <svg v-if="!isError" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
          </svg>
          <svg v-else xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01M12 5.5l7.5 13h-15L12 5.5z" />
          </svg>
          <span>{{ message }}</span>
        </div>
...
<style scoped>
.fade-in-enter-active, .fade-in-leave-active { transition: opacity .25s, transform .25s; }
.fade-in-enter-from, .fade-in-leave-to { opacity: 0; transform: translateY(4px); }
</style>
