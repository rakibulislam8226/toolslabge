<template>
    <button :type=" type " :class=" buttonClasses " :disabled=" disabled || loading " @click=" handleClick ">
        ...
        <!-- Icon Slot -->
        <span v-if="$slots.icon && !loading" class="flex items-center" :class=" { 'mr-2': $slots.default || label } ">
            <slot name="icon"></slot>
        </span>
...
const props = defineProps({
    ...
    variant: {
        type: String,
        default: 'primary',
        validator: (value) => ['primary', 'secondary', 'danger', 'success', 'ghost', 'outline', 'icon'].includes(value)
    },
    size: {
        type: String,
        default: 'md',
...
const buttonClasses = computed(() => {
    const classes = [
        ...
        (props.disabled || props.loading) ? 'disabled:opacity-50' : ''
    ]
...
const roundedClasses = computed(() => {
    ...
    return roundedMap[props.rounded] || roundedMap.md
})
...
const variantClasses = computed(() => {
    const variants = {
        ...
        success: [
            ...
            'active:bg-green-800 active:shadow-inner',
        ...
        icon: [
            ...
            'dark:hover:text-gray-200 dark:active:bg-gray-700',
            'dark:focus:ring-gray-400'
        ].join(' ')
    }
